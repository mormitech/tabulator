...
export default class Edit extends Module {
	...
	initializeColumn(column) {
		var convertEmpty = Object.keys(column.definition).includes(
			"editorEmptyValue"
		);
	...
	bindEditor(cell) {
		if (cell.column.modules.edit) {
			...
			if (
				this.options("editTriggerEvent") === "focus" ||
	...
	transformEmptyValues(value, cell) {
		...
		if (convert) {
			checkFunc =
				mod.editorEmptyValueFunc || this.options("editorEmptyValueFunc");

			if (checkFunc && checkFunc(value)) {
				value = mod.convertEmptyValues
					? mod.editorEmptyValue
